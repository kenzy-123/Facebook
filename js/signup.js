...
function validationSign(){
   if(fname.value == "" || fname.value.length < 3 || fname.value.includes(" ")){
        ...
        return false;   
   }else if(email.value == "" || !email.value.includes("@") || !email.value.includes(".") || email.value.includes(" ") || !email ){
        setSuccess();
        setError(emaillabel , "Invalid Email &#9940;");
        return false;
   } else if(phone.value == "" || phone.value.length != 11 || phone.value.includes(" ") || isNaN(phone.value)){
        setSuccess();
        setError(phonelabel , "Invalid Phone Number &#9940;");
        return false; 
   } else if(password.value == "" || password.value.length < 8 || !password.value.includes("!")){
        setSuccess();  
        ...
        setSuccess();
        setError(genderlabel , "Please select a gender &#9940;");
        return false;
